...
//作成した二つのMapperをフィールドとして追加、ファイルを保存する
	@Autowired
...
//ブラウザからLoginFormを介して情報を受け取り、welcome.htmlに遷移させる
	//PostMappingはpost通信のみを受け取る, welcomeメソッドは「ログイン時」に呼び出される。(RequestMappingはget通信も可能)
	@PostMapping("/welcome")
	public String welcome(LoginForm form, Model model) {
		
		//MstUserMapperのこのメソッドを呼び出す。userにはSQLの実行結果が代入される
		//？なぜMstUserMapper.findBy~としないのか？　findBy~はMstUserMapperの中にあるはずなのに。
		...
		//MstGoodsMapperのfindAllメソッドを呼び出し。得た商品情報をmodelに登録する
